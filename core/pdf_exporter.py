...
def merge_html_slides(output_dir="output", num_slides=16):
...
async def export_to_pdf_async(html_file):
...
def export_to_pdf_sync(html_file):
    """Sync wrapper for async PDF export"""
    import asyncio
    return asyncio.run(export_to_pdf_async(html_file))


def merge_and_export_pdf(output_dir="output", num_slides=16):
    """Complete workflow: merge HTML slides and export to PDF"""
    print("🚀 MERGE & EXPORT PDF")
    merged_file = merge_html_slides(output_dir, num_slides)
    if not merged_file:
        return None, None
    pdf_file = export_to_pdf_sync(merged_file)
    if pdf_file:
        print("✅ DONE!")
        return merged_file, pdf_file
