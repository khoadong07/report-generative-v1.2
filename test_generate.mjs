...
`;
    ...
    for (let idx = 0; idx < slideFiles.length; idx++) {
        ...
        if (slideContainer.length > 0) {
            ...
            if (idx > 0) {
                // Find all canvas elements and update their IDs
                slideContainer.find('canvas').each((i, canvas) => {
                    const oldId = $(canvas).attr('id');
                    if (oldId) {
                ...
                // Update all element IDs in the slide container
